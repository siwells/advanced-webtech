...
\paragraph{} Now create a simple HTML template, called hello\_template.html inside the templates folder, e.g.
\begin{lstlisting}[style=DOS]
$ touch templates/hello_template.html
\end{lstlisting}

\paragraph{} Now open hello\_template.html in Vim for editing, e.g.
\begin{lstlisting}[style=DOS]
$ vim templates/hello_template.html
\end{lstlisting}
...
\paragraph{} What we have just done is create a template that is a mix of Jinja2 tags, indicated by the \{\%, \}\%, \{\{, and \}\} tags, and HTML tags, indicated nby the $<$ and $>$ tags that we are already used to. The HTML tags are rendered as you would expect regular HTML to be treated, but the Jinja2 tags cause conditional behaviour to occur; in this case they form an if...else clause, just like we have seen in many other procedural languages. We also have a variable placeholder in the \{\{name\}\} clause which means that we can supply a variable to replace the name placeholder with. Let's use our template now in a quick Flask app:
