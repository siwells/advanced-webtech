...
\paragraph{} Make yourself a similar directory hierachy to that shown above then download Bootstrap from \url{https://github.com/twbs/bootstrap/releases/download/v3.3.5/bootstrap-3.3.5-dist.zip} and unpack it, e.g.
...
\paragraph{} Now move the bootstrap files into directory hierarchy that you created. We can now start to use bootstrap to provide some style. The first thing to do is to create a simple Flask app that returns an HTML page using a template. We will then extend the template to add some bootstrap functionality to it. So, let's start with the basic template:

\begin{lstlisting}
FLASK TEMPLATE LISTING
\end{lstlisting}
...
% Add screenshot of before bootstrap

Let's use Flask to add some basic styling:

\begin{lstlisting}
WITH ADDED BOOTSTRAP
\end{lstlisting}
