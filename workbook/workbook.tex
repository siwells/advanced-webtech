...
\paragraph{} Create a sub-directory called `etc' just like you did for the static and templats directories. Your config files will live here. Now create a new text file in the etc directory called `defauts.cfg', this will be the default file that your flask app reads in at start up. We can now use that file to store some configuration data. Let's start by moving our debug and host settings into the config file instead of setting them in code, e.g. Add the following to your defaults.cfg:
...
\paragraph{} Now create a configuration.py file in the parent of the etc directory. This file will read from the default.cfg file and provide the information to our main flask app, e.g.
...
\paragraph{} Notice how we are `getting' the value for each key from the config file then storing those values in the app.config object. We can now create a Flask app that uses those values, for example,
...
\section{Logging}
\label{logging}
\paragraph{} We can set our Flask app up to record interesting happenings into a text file so that we have a log to inspect if something bad occurs. Let's create a new sub-directory, next to `etc' and let's call it `var'. This directory will be the location that our new log files will write to. First we need to make some additions to our config file, e.g.
...
level = DEBUG
\end{lstlisting}
\paragraph{} Notice the logging section towards the end which defines the name of the log file `'loggingapp.log' the location, our var/ directory that we just created, and the log level, the granularity of the logging events to record.

\paragraph{} We can now extend our configuration.py file to use the new logging settings and initialise our logging:
...
\end{lstlisting}
\paragraph{} A lot of these setting, especially in the logs function are merely setting up the parameters of the logging system. There are many parameters to fine tune how data is logged so if you want to make changes then you will have to dig into the Python logging documentation but for now, just accept the defaults as they produce files that cope well with lots of data, are easy to read, and which can be automatically processed.

\paragraph{} Now all we need to do is initialise all these settings and start them going from our Flask app.
...
if __name__ == '__main__':
    app.run(
        ...
        threaded=app.config['threaded'])
\end{lstlisting}
\paragraph{} All we have had to do to use the logger is to initialise the logging system by calling our configuration modules logs() function. From then on we can actually log messages using the app.logger.warn() and passing in a String. NB. There are also other logging levels suchas debug or error that we can use to distinguish the importance of different messages in the logs. Investigate the Python logging documentation to find out more. 
